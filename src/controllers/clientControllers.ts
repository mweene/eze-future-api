...
function getAllClients(req, res) {
  try {
    ...
    const clients = getAllClientsQuery.all() as ClientDB[];

    // Build complete client objects with related data
    const completeClients: ClientResponse[] = clients.map((client) => {
      const plotDetails = getPlotDetailsByClientId.get(client.id) as
        | PlotDetailsDB
        | undefined;
      const witness = getWitnessByClientId.get(client.id) as
        | WitnessDB
        | undefined;
      const documents = getDocumentsByClientId.get(client.id) as
        | DocumentsDB
        | undefined;
      ...
      if (documents) {
        clientData.documents = {
          ...
          authorizationLetterLink:
            documents.authorization_letter_link || undefined,
        };
      }
    ...
    const insertClientTransaction = db.transaction((data) => {
      ...
      if (data.plotDetails) {
        ...
        insertPlotDetails.run(
          ...
          plot.siteName || null,
          plot.grandPrice || null,
          plot.amountPaid || null,
          plot.balance || null,
          plot.allocated || null,
          plot.allocationDate || null,
          plot.paymentStatus || null,
          plot.dateBought || null,
          plot.dateUpdated || null,
        );
      }
      ...
      if (data.documents) {
        ...
        insertDocuments.run(
          clientId,
          docs.nrcLink || null,
          docs.letterOfSaleLink || null,
          docs.landAgreementLink || null,
          docs.allocationFormLink || null,
          docs.authorizationLetterLink || null,
        );
      }
...
function getClientById(req, res) {
  try {
    ...
    // Get client data
    const client = getClient.get(clientId) as ClientDB | undefined;
    ...
    // Get related data
    const plotDetails = getPlotDetails.get(clientId) as
      | PlotDetailsDB
      | undefined;
    const witness = getWitness.get(clientId) as WitnessDB | undefined;
    const documents = getDocuments.get(clientId) as DocumentsDB | undefined;
