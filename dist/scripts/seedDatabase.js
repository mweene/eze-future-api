...
function seedDatabase() {
    ...
    try {
        ...
        try {
            ...
            console.log("Note: sqlite_sequence table not found (this is normal for empty databases)");
        }
    ...
    `);
        ...
        const insertClientTransaction = db.transaction((clientData) => {
            ...
            const clientInfo = insertClient.run(clientData.name, clientData.sex, clientData.nrc, clientData.phone, clientData.email, clientData.address);
            const clientId = clientInfo.lastInsertRowid;
            // Insert plot details
            if (clientData.plotDetails) {
                const plot = clientData.plotDetails;
                insertPlotDetails.run(clientId, plot.plotSize, plot.plotNumber, plot.siteName, plot.grandPrice, plot.amountPaid, plot.balance, plot.allocated, plot.allocationDate, plot.paymentStatus, plot.dateBought, plot.dateUpdated);
            }
            // Insert witness information
            if (clientData.witness) {
                const witness = clientData.witness;
                insertWitness.run(clientId, witness.name, witness.sex, witness.nrc, witness.email, witness.phone, witness.address, witness.relationship);
            }
            // Insert documents
            if (clientData.documents) {
                const docs = clientData.documents;
                insertDocuments.run(clientId, docs.nrcLink, docs.letterOfSaleLink, docs.landAgreementLink, docs.allocationFormLink, docs.authorizationLetterLink);
            }
            return clientId;
        });
